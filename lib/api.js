...
function verifyParameters(operation, actual) {
	...
	return new Promise((resolve, reject) => {
		...
		const expectedParameters = operation.parameters || [];
		...
		for(var paramName in actual) {
			var value = actual[paramName];
			var expectedParam = expectedParameters.filter(p => p.name == paramName);
		...
		if (missingParameters.length !== 0) {
			if (missingParameters.length > 1) {
				reject(new Error(`Required parameters ${JSON.stringify(missingParameters)} were not present in "${operation.serviceName}" "${operation.name}" request.`));
			} else {
				reject(new Error(`Required parameter "${missingParameters[0].name}" was not present in "${operation.serviceName}" "${operation.name}" request`));
			}
		} else {
			resolve();
		}
